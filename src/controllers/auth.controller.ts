  ...
  try {
    ...
    if (isPrismaClientKnownRequestError(error)) {
      if (error.code === PrismaErrorCode.UniqueConstraintViolation) {
        throw new EntityError([{ message: 'Email đã tồn tại' }])
      }
    }
    throw error
  }
...
export const loginController = async (body: LoginBodyType) => {
  ...
  if (!account) {
    throw new EntityError([{ message: 'Email không tồn tại' }])
  }
  const isPasswordMatch = await comparePassword(body.password, account.password)
  if (!isPasswordMatch) {
    throw new EntityError([{ message: 'Email hoặc mật khẩu không đúng' }])
  }
  const sessionToken = signSessionToken({
    userId: account.id
  })
